...
|--------------|--------------------------------|---------------------------------------------------------------------------------------------------|
| **Trace**    | `query_traces`                 | Query traces with multi-condition filtering (service, endpoint, start, end, state, tags). Supports `full`, `summary`, and `errors_only` views with performance insights. |
